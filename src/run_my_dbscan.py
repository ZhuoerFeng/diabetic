...
def main():
    ...
    print(new_data.mean())
    start = time.time()
    # clustering = DBSCAN(eps=0.15, min_samples=10).fit(new_data.to_numpy()) # 558000
    clustering = FastDBSCAN(eps=0.15, min_samples=100).fit(new_data.to_numpy()) # 558000
    ...
    write_results_to_local_csv(
        ...
        filename="my_dbscan_results.csv",
        overwrite=False,
        append=False
    )
